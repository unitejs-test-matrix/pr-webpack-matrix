...
node unitejs/cli/bin/unite configure --packageName=pr-wpk-ts-jas-pro --title="Preact Webpack TypeScript" --sourceLanguage=TypeScript --moduleType=CommonJS --bundler=Webpack --unitTestRunner=Karma --unitTestFramework=Jasmine --unitTestEngine=PhantomJS --e2eTestRunner=Protractor --e2eTestFramework=Jasmine --linter=TSLint --license=MIT --packageManager=Npm --cssPre=Css --cssPost=None --appFramework=Preact --disableVersionCheck --outputDirectory=./apps/pr-wpk-ts-jas-pro
